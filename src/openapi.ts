...
function makeOpenApi(options: {
	...
	for (let api of options.apis) {
		for (let endpoint of api.definitions as (ZodiosEndpointDefinition<unknown> & {
			example: any;
			examples: { [key: string]: { value: any } };
		})[]) {
			...
			for (let error of (endpoint.errors ?? []) as (ZodiosEndpointError<unknown> & {
				example: any;
				examples: { [key: string]: { value: any } };
			})[]) {
			...
			if (pathParams) {
				for (let pathParam of pathParams) {
					...
					const param = findPathParam(endpoint, paramName) as
						| (ZodiosEndpointParameter<unknown> & {
								example?: any;
								examples?: { [key: string]: { value: any } };
						  })
						| undefined;
			...
			for (let param of (endpoint.parameters ?? []) as (ZodiosEndpointParameter<unknown> & {
				example: any;
				examples: { [key: string]: { value: any } };
			})[]) {
				if (!expludedParamTypes.includes(param.type)) {
					...
					const schema =
						required || !isZodType(param.schema, z.ZodFirstPartyTypeKind.ZodOptional)
							? param.schema
							: (param.schema as z.ZodOptional<z.ZodType>).unwrap();
					parameters.push({
						name:
							param.type === "Query" && isZodType(param.schema, z.ZodFirstPartyTypeKind.ZodArray)
								? `${param.name}[]`
								: param.name,
			...
			const body = endpoint.parameters?.find((param) => param.type === "Body") as
				| (ZodiosEndpointParameter<unknown> & {
						example: any;
						examples: { [key: string]: { value: any } };
				  })
			...
			const operation: OpenAPIV3.OperationObject = {
				...
				security:
					"scheme" in api && api.scheme ? [{ [api.scheme]: api.securityRequirement ?? ([] as string[]) }] : undefined,
				requestBody: body
					? {
...
export function toOpenApi(
	...
	return makeOpenApi({
		...
		securitySchemes: options?.securityScheme ? { auth: options.securityScheme } : undefined,
		...options,
	});
}
